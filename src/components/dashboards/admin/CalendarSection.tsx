...
export default function CalendarSection({ profile }: Props) {
  ...
  const [form, setForm] = useState<{ title: string; description: string; start_date: string; end_date: string; event_type: EventType }>({
    title: '', description: '', start_date: new Date().toISOString().split('T')[0], end_date: '', event_type: 'general',
  });
  ...
  const save = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ title: '', description: '', start_date: new Date().toISOString().split('T')[0], end_date: '', event_type: 'general' });
      fetchEvents();
    } catch (e: unknown) {
      setToast({ msg: e instanceof Error ? e.message : 'Failed to add event', type: 'error' });
    }
    setSaving(false);
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => { setForm({ title: '', description: '', start_date: new Date().toISOString().split('T')[0], end_date: '', event_type: 'general' }); setShowModal(true); }} className="flex items-center gap-1.5 px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="flex gap-3 p-5 border-t border-gray-100">
              ...
              <button onClick={save} disabled={saving} className="flex-1 py-2.5 bg-teal-600 text-white rounded-xl text-sm font-medium hover:bg-teal-700 disabled:opacity-50">{saving ? 'Saving...' : 'Add Event'}</button>
            </div>
          </div>
        </div>
      )}
